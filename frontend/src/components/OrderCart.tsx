const OrderCart = ({ order, setShowFAQ }: any) => {
  return (
      ...
      <div className="flex flex-col gap-[16px] px-4 md:px-8 py-4 ">
        <div className="flex flex-wrap gap-[14px] items-start justify-between ">
          ...
          <div
            ...
            className="flex cursor-pointer bg-white rounded-[28.017px] border-[#D9D9D9] border-solid border-[1px] items-center justify-center gap-[9.339px] py-2 px-6  md:py-[7px] md:px-[49px] text-[12px] md:text-[14px] font-medium"
          >
            Order Details
          </div>
        </div>
        ...
        <div className="inline-flex flex-wrap  items-center gap-6">
          {order.images.map((image: string) => (
            <img key={image} src={image} className="bg-no-repeat bg-cover" />
          ))}
        </div>
      </div>
    </div>
